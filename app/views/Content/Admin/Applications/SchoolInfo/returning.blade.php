...
<div id="educationalInfo">
	{{ Form::open(array('url' => route('doSchoolInfo', array($appKey)), 'method' => 'POST', 'accept-charset' => 'UTF-8')) }}
	{{ Form::hidden('type', $type)}}

	<div id="returning_student" class="panel panel-primary collegeInfo">
		<div class="panel-heading">College Information - Returning Students <font color="orange">{{ $errors -> first('major')}}</font> <font color="orange">{{ $errors -> first('creditsEarned')}}</font> <font color="orange">{{ $errors -> first('GPA')}}</font> <font color="orange">{{ $errors -> first('collegeGrad')}}</font></div>
		<ul class="panel-body">
			<li>
				<label for="major">Major:</label>				
				<br>
				<input placeholder="Major" autocomplete="off" name="major" type="text" value="{{{$edu->major or Input::old('major')}}}" id="major">
				<br>
			</li>
			...
			<li>
				...
				<br>
				<input placeholder="Credits" autocomplete="off" name="creditsEarned" type="text" value="{{{$edu->creditsEarned or Input::old('creditsEarned')}}}" id="creditsEarned">
				<br>
			</li>
			...
			<li>
				...
				<br>
				<input placeholder="GPA" autocomplete="off" name="GPA" type="text" value="{{{$edu->GPA or Input::old('GPA')}}}" id="GPA">
				<br>
			</li>
		</ul>
	</div>
